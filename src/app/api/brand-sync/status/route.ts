...
import { getSyncGroupForTenant, listPendingChanges } from "@/lib/brandSync";
...
export async function GET(req: Request) {
  try {
    ...
    const sync = await getSyncGroupForTenant(access.tenantId);
    if (!sync) {
      return NextResponse.json({ linked: false, group: null, pendingCount: 0 });
    }

    const pending = await listPendingChanges(access.tenantId);
    return NextResponse.json({
      linked: true,
      group: {
        id: sync.groupId,
        name: sync.name,
        approvalMode: sync.approvalMode,
