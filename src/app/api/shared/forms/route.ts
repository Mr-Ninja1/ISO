...
export async function POST(req: Request) {
  ...
  const mode = body.mode === "today" || body.mode === "all" ? body.mode : "selected";
  const inputAuditIds = Array.isArray(body.auditIds)
    ? body.auditIds.filter((value): value is string => typeof value === "string" && value.trim().length > 0)
    : [];
  ...
  if (membershipErr || !membership || !hasPermission(membership.role, "audit.view")) {
  ...
  if (!auditIds.length) {
    return NextResponse.json({ error: "No submitted forms selected to share" }, { status: 400 });
  }
  ...
  if (auditsErr) return NextResponse.json({ error: auditsErr.message }, { status: 500 });
  const validAuditIds = (allowedAudits || []).map((row) => row.id as string);
  if (!validAuditIds.length) {
    return NextResponse.json({ error: "No valid submitted forms found to share" }, { status: 400 });
  }
  ...
  if (linkErr || !link) {
    return NextResponse.json({ error: linkErr?.message || "Failed to create shared link" }, { status: 500 });
  }
  ...
  const { error: itemsErr } = await svc.from("shared_form_link_items").insert(items);
  if (itemsErr) {
    await svc.from("shared_form_links").delete().eq("id", link.id);
    return NextResponse.json({ error: itemsErr.message || "Failed to attach forms to shared link" }, { status: 500 });
  }
