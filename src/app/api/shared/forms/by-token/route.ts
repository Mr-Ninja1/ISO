...
export async function GET(req: Request) {
  ...
  const { data: link, error: linkErr } = await svc
    ...
    .select("id, tenant_id, title, mode, is_active, expires_at, created_at, tenants(name,slug,logo_url)")
    .eq("token", token)
    .maybeSingle();

  if (linkErr || !link) return NextResponse.json({ error: "Shared link not found" }, { status: 404 });
  if (link.is_active === false) return NextResponse.json({ error: "This shared link has been disabled" }, { status: 410 });
  ...
  const { data: items, error: itemsErr } = await svc
    ...
    .select("sort_order, audit_id, audit_logs(id, status, template_id, created_at, updated_at, submitted_at, form_templates(title))")
    .eq("link_id", link.id)
  ...
  const rows = (items || []).flatMap((item) => {
    const row = Array.isArray(item.audit_logs) ? item.audit_logs[0] : item.audit_logs;
    if (!row) return [];
